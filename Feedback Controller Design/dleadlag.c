...
/*
	discrete time leadlag filter (Matlab 6 version)
	...
	Parameter: [f_num f_den] (frequency in Hz)
*/

#define S_FUNCTION_NAME dleadlag
...
#define F_DEN		ssGetSFcnParam(S,1)
...
static void mdlInitializeSampleTimes(SimStruct *S)
{
	if (ssIsVariableStepSolver(S)) {
		...
		printf("dleadlag: discete stepsize = %f s.\n",get_step_size(S));
#endif
	} else {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	fden=absval(*param_fden);

/*	parameters are (re)computed here to enable changes on-the-fly */

	eps=1.0e-16;
	/* omp=2*PI*maxval(fnum,fden)+eps; */
    omp=2.0*PI*sqrt(fnum*fden)+eps;
